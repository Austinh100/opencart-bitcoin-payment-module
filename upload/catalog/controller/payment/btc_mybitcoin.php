...
class ControllerPaymentBTCmybitcoin extends Controller {
	...
	protected function index() {
		...
		$this->data['order_total']		=$this->currency->format($order['total'], $order['currency_code'], $order['currency_value'], false);
		$this->data['order_currency']	=$order['currency_code'];
		
		if ($order['currency_code']!='BTC') {
		$result=mbc_getrates();
		if ($result['SCI_Reason'] && $result['SCI_Currency_'.$order['currency_code'].'_Rate']) {
			$this->data['total_btc']		=round($order['total']/$result['SCI_Currency_'.$order['currency_code'].'_Rate'],2);
			$this->data['btc_rate']			=round($result['SCI_Currency_'.$order['currency_code'].'_Rate'],3);
			$this->data['main_currency']	=$order['currency_code'];
		}
}
		...
		*/
		...
		$plaintext_querystring=
		"amount="			.$order['total']*$order['currency_value'].			
